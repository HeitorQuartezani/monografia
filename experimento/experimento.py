# ==================================================================================================
# SCRIPT DE EXECUÇÃO DE EXPERIMENTO (v4.3 - Testando System Prompts)
#
# DESCRIÇÃO:
# - ALTERADO: Remove o teste de 'temperature' (que é de baixo impacto).
# - ADICIONADO: Testa duas variações de 'system_prompt_override' (alto impacto).
# - Mantém a autenticação por Chave de API.
# - Mantém a arquitetura de duas fases (Recuperação -> Geração).
# ==================================================================================================
import json
import time
import pandas as pd
...
API_URL_RECUPERAÇÃO = f"{API_URL_BASE}/recuperar_contexto"
...
if not API_KEY:
    print("="*80)
    print("ERRO CRÍTICO: 'API_KEY' não encontrada no arquivo .env.")
    print("="*80)
    exit()
...
PARAMETROS_RECUPERAÇÃO = {
    "chunking_strategy": ["recursive_1000_200", "recursive_500_100","semantic_percentile_75", "semantic_percentile_95"],
    "search_type": ["vetorial", "textual", "hibrida"],
}

# --- 2. ALTERAÇÃO: Parâmetros de Geração ---
# Removemos 'temperature' e adicionamos 'system_prompt_override'
TEMPERATURA_FIXA = 0.1 # Temperatura agora é fixa
...
TOP_K_VALORES = [5, 10, 15, 20]
MAX_K = max(TOP_K_VALORES) 
...
def chamar_api_recuperacao(config_recuperacao: dict, pergunta_str: str):
    ...
    try:
        response = requests.post(API_URL_RECUPERAÇÃO, 
                                 json=payload, 
                                 timeout=120, 
                                 headers=API_HEADERS) # Já inclui Auth
...
def fase_1_recuperacao():
    ...
    plano_recuperacao = gerar_plano_experimental(PARAMETROS_RECUPERAÇÃO)
        ...
        for config_rec in pbar_configs:
            ...
            for item_gabarito in tqdm(GABARITO, desc=f"Config Rec: {str(config_rec)}", leave=False):
                ...
                resultado_recuperacao = {
                    ...
                    "expected_answer": item_gabarito.get("ground_truth_answer"),
...
def carregar_resultados_existentes_csv(caminho_arquivo: str, colunas_chave: list) -> set:
    ...
    try:
        ...
        # O código aqui é robusto. Ele vai automaticamente procurar
        # pela coluna 'system_prompt_override' agora, pois ela está em PARAMETROS_GERACAO.keys()
...
# --- 3. ALTERAÇÃO: Lógica da chamada de Geração ---
def chamar_api_geracao(pergunta_str: str, contexto_str: str, config_geracao: dict):
    """
    config_geracao agora contém 'model' e 'system_prompt_override'.
    'temperature' é adicionada manualmente.
    """
    payload = {
        "pergunta": pergunta_str,
        "contexto": contexto_str,
        "temperature": TEMPERATURA_FIXA, # Adiciona a temperatura fixa
        **config_geracao                 # Adiciona 'model' e 'system_prompt_override'
    }
    try:
        response = requests.post(API_URL_GERACAO, 
                                 json=payload, 
                                 timeout=180, 
                                 headers=API_HEADERS) # Já inclui Auth
...
def fase_2_geracao():
    print("\n--- INICIANDO FASE 2: GERAÇÃO DE RESPOSTAS (TESTANDO PROMPTS) ---")
    ...
    # Esta lógica é automática e robusta, vai incluir 'system_prompt_override'
    colunas_config_chave = sorted(list(PARAMETROS_RECUPERAÇÃO.keys()) + list(PARAMETROS_GERACAO.keys()) + list(PARAMETROS_FIXOS.keys()) + ["top_k", "question"])
        ...
        for line in pbar_recuperacao:
            ...
            for k in TOP_K_VALORES:
                ...
                for config_gen in plano_geracao:
                    # config_completa agora contém 'system_prompt_override'
                    config_completa = {
                        **resultado_rec["config_recuperacao"], 
                        **config_gen, 
                        "top_k": k, 
                        "question": resultado_rec["question"],
                        "temperature": TEMPERATURA_FIXA # Adiciona a temp fixa aos resultados
                    }
                    
                    # Chave de teste agora inclui o prompt
                    chave_teste_atual = tuple(config_completa[col] for col in colunas_config_chave)
                    
                    if chave_teste_atual in geracoes_ja_feitas:
                        continue
                        
                    # config_gen (que tem 'model' e 'system_prompt_override') é passado
                    ...
                    resultado_final_completo = {
                        **config_completa,
                        ...
                        "full_context_sent": contexto_str
                    }
    ...
    # Garante que 'temperature' e 'system_prompt_override' apareçam nas colunas de parâmetros
...
def main():
    print("Iniciando experimento v4.3 (Testando System Prompts com Autenticação)")
    fase_1_recuperacao()
    fase_2_geracao()
